...
class AttendanceSheet(models.Model):
    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            reviewd_attendences = att_sheet.line_ids
            ...
            abs_cnt = 0
            late_cnt = []
            is_flexable = att_sheet.contract_id.resource_calendar_id.is_flexable
            flexable_hour = att_sheet.contract_id.resource_calendar_id.planned_hour
            float_flexable_hour = (flexable_hour * 60 * 60) / 3600
            for day in all_dates:
                day_start = datetime(day.year, day.month, day.day)
                day_end = day_start.replace(hour=23, minute=59,
                                            second=59)
                reviewed = 0
                for i in reviewd_attendences:
                    if i.date == day:
                        reviewed = 1
                if reviewed:
                    continue
                    ...
                    if public_holiday:
                        if attendance_intervals:
                            for attendance_interval in attendance_intervals:
                                ...
                                overtime_policy = policy_id.get_overtime(self._get_float_from_time(
                                    ac_sign_in))
                        ...
                        for i, work_interval in enumerate(work_intervals):
                            ...
                            ac_sign_in = 0
                            overtime_policy = policy_id.get_overtime(0)
                                ...
                                if len(att_work_intervals) > 1:
                                    ...
                                    ac_sign_in = self._get_float_from_time(
                                        pytz.utc.localize(att_work_intervals[0][
                                                              0]).astimezone(
                                            tz))
                                    overtime_policy = policy_id.get_overtime(ac_sign_in)
                                    ac_sign_out = self._get_float_from_time(
                                        pytz.utc.localize(att_work_intervals[0][
                                                              1]).astimezone(
                                            tz))
                            ...
                            if float_overtime <= overtime_policy['wd_after']:
                                ...
                                float_overtime = float_overtime * \
                                                 overtime_policy[
                                                     'wd_rate']
                                float_overtime = policy_id.get_wd_overtime(pytz.utc.localize(overtime_interval[0]).astimezone(
                                            tz),pytz.utc.localize(overtime_interval[1]).astimezone(
                                            tz))

                            float_late = late_in.total_seconds() / 3600
                            act_float_late = late_in.total_seconds() / 3600
                            late_hours = float_flexable_hour - float_worked_hours
                            policy_late, late_cnt = policy_id.get_late(
                                float_late,
                                late_cnt, is_flexable, late_hours)
    ...
    def _get_workday_lines(self):
        ...
        if not loans_work_entry:
            raise ValidationError(_(
                'Please Add Work Entry Type For Attendance Sheet Loans With Code ATTSILOANSDED'))
        ...
        worked_days_lines = overtime + late + absence + difftime + loans_deduction
    ...
    def create_payslip(self):
        ...
        for att_sheet in self:
            ...
            worked_days_line_ids += overtime + late + absence + difftime + loans_deduction
...
class AttendanceSheetLine(models.Model):
    ...
    @api.depends('att_sheet_id.contract_id', 'status')
    def _compute_planned_hour(self):
        for rec in self:
            rec.planned_worked_hour = 0
            if rec.att_sheet_id.contract_id.resource_calendar_id.is_flexable and not rec.status:
                rec.planned_worked_hour = rec.att_sheet_id.contract_id.resource_calendar_id.planned_hour
