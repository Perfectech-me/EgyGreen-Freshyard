        ...
        if self.move_type == 'in_invoice' and not self.bill_sequence and self.rate_without_holding > 0:
            ...
            if self.bill_sequence:
                ...
                if len(bill_sequence_year) == 3:
                    self.bill_sequence_year = str(bill_sequence_year[0]) + "/" + str(bill_sequence_year[2]) + "/" + str(
                        bill_sequence_year[1])
                else:
    ...
    def _check_bl_awb_form13number(self):
        for rec in self:
            for move in self.search(
                    [('id', '!=', rec.id), ('move_type', '=', 'out_invoice'), ('company_id', '=', rec.company_id.id)]):
