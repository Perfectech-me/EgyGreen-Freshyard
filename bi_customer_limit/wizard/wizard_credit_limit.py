...
#         self.base_url = self.env.context.get('base_url')
    ...
    def confirm_sale(self):
        ...
        partner_id.sudo().write({'credit_limit' : self.customer_credit_limit})
        ...
        for partner in partner_id:
            if partner:
                ...
                if template_id:
                    values = email_template_obj.sudo().generate_email(active_ids[0], ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])

                    email_values = {}
                    email_values['email_to'] = su_id.sudo().email
                    email_values['email_from'] = partner.sudo().email
                    email_values['subject'] = values.get('subject') or ''
                    email_values['body_html'] = values.get('body_html') or ''
                    email_values['res_id'] = False
                    email_values['author_id'] = self.env['res.users'].sudo().browse(self._uid).partner_id.id
